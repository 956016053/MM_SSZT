...
window.gachaSystem = {
    ...
    resetProgress() { if(confirm("确定重置？")) { localStorage.removeItem('ai_gacha_save'); location.reload(); } }
};
...
window.gestureSystem = {
    ...
    async toggleCamera() {
        if (location.protocol === 'file:') return alert("❌ 请在 HBuilderX 点击【运行到浏览器】！");
            ...
            if(result.landmarks.length > 0) {
                ...
                if(this.lastX !== 0 && !this.cooldown) {
                    ...
                    if (absX > THRESHOLD || absY > THRESHOLD) {
                        if (absY > absX) {
                            if (deltaY < -THRESHOLD) this.triggerAction("⬆️ 向上: 全翻", () => window.gachaSystem.flipAll());
                            else this.triggerAction("⬇️ 向下: 抽卡", () => window.gachaSystem.draw(5));
                        } else {
                            if (deltaX > THRESHOLD) this.triggerAction("➡️ 向右: 下一张", () => window.gachaSystem.selectNext());
                            else this.triggerAction("⬅️ 向左: 翻转", () => window.gachaSystem.flipCurrent());
                        }
                    }
                }
